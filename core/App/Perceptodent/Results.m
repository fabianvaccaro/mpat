...
% Last Modified by GUIDE v2.5 13-Oct-2015 13:20:50
...
%Muestra el mejor IPM
if(nargin > 3)
    for index = 1:2:(nargin-3),
        if nargin-3==index, break, end
        switch varargin{index}
         case 'StRes'
             handles.StRes = varargin{index+1};
         case 'TablaFinal'
             handles.TablaFinal = varargin{index+1};
        end
    end
end

%mostrar resultados
set(handles.OptimalMP, 'String', char(handles.StRes.designacion(handles.StRes.Optimal,1)));
set(handles.lbl_rho, 'String', handles.StRes.SP_RHO(handles.StRes.Optimal));
set(handles.lbl_bonff, 'String', handles.StRes.GAMMAZ(handles.StRes.Optimal));
axes(handles.axes1);
Y = handles.StRes.descriptivos(:,1,handles.StRes.Optimal);
X = handles.StRes.Clases;
E = handles.StRes.descriptivos(:,3,handles.StRes.Optimal);
errorbar(X,Y,E);
TituloGrafico = ['Graph of',' ', char(handles.StRes.designacion(handles.StRes.Optimal,2)), ' ', '(',char(handles.StRes.designacion(handles.StRes.Optimal,1)),')' ];
title(TituloGrafico);
xlabel('Number of chewing cycles'); % x-axis label
ylabel(char(handles.StRes.designacion(handles.StRes.Optimal,1))); % y-axis label
